...
# =========================
def is_sacc_bad(gspeed_mmps: int, sacc_mmps: int) -> bool:
    if gspeed_mmps <= 5555:
        return sacc_mmps > 278
    return float(sacc_mmps) > 0.05 * float(gspeed_mmps)
...
class UbxNmeaSimulator:
    ...
    def build_nav_pvt(self) -> bytes:
        ...
        valid_flag = 0x07
        if self.current_profile in ("NO_FIX", "GPS_DISABLE"):
            valid_flag = 0x00
        payload += struct.pack(
            "<HBBBBBB",
            now.year,
    ...
    def build_nmea_gga(self) -> str:
        ...
        hdop = f"{random.uniform(0.5, 50.0):.1f}"
        alt = f"{self.h_m:.1f}"
        geoid = "0.0"
        parts = [
            "GNGGA",
...
class SimulatorGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.lbl_sacc = ttk.Label(
            stat,
            text="sAcc: —",
            font=("Segoe UI", 11, "bold"),
            foreground=self.col_gray,
        )
        self.lbl_sacc.grid(row=3, column=0, sticky="w")
    ...
    def _clear_status_and_tx(self):
        ...
        self.lbl_sacc.config(text="sAcc: —", foreground=self.col_gray)
    ...
    def _parse_nav_pvt_kmh(self, ubx_hex: str, return_extras: bool = False):
        """Parse UBX-NAV-PVT. Default returns km/h.
        If return_extras=True -> returns (kmh, gspeed_mmps, sacc_mmps)."""
        try:
            b = bytes.fromhex(ubx_hex)
            if len(b) < 8 or b[0] != 0xB5 or b[1] != 0x62:
                return None
            ...
            if len(b) < 6 + plen + 2:
                return None
            payload = b[6 : 6 + plen]
            # gSpeed at 60..63 (int32, mm/s)
            if len(payload) < 64:
                return None
            gspeed_mms = int.from_bytes(payload[60:64], "little", signed=True)
            kmh = max(0.0, (gspeed_mms / 1000.0) * 3.6)
            # sAcc at 68..71 (uint32, mm/s) — may be absent if payload shorter
            sacc_mms = None
            if len(payload) >= 72:
                sacc_mms = int.from_bytes(payload[68:72], "little", signed=False)
            if return_extras:
                return (kmh, gspeed_mms, sacc_mms)
            return kmh
        except Exception:
            return None
            ...
            while True:
                ...
                for key, label in (
                    ...
                    if key in ubx_map and key in self.ubx_tree.get_children():
                        ...
                        if key == "pvt":
                            parsed = self._parse_nav_pvt_kmh(hexstr, return_extras=True)
                            if (
                                parsed is not None
                                and self._current_profile_name
                                not in ("NO_FIX", "GPS_DISABLE")
                            ):
                                kmh, g_mms, s_mms = (
                                    parsed  # s_mms may be None if payload shorter than 72
                                )
                                # ---- update Speed & Effective offset (unchanged logic) ----
                                self._last_speed_kmh = float(kmh)
                                raw = self._parse_int_str(self.calib_var.get())
                                ...
                                # ---- update sAcc value + color if available ----
                                if s_mms is not None:
                                    bad = is_sacc_bad(g_mms, s_mms)
                                    # If bạn dùng 2 label (title/value) như patch trước:
                                    # self.lbl_sacc_title.config(foreground=self.col_text)
                                    # self.lbl_sacc_value.config(text=f"{s_mms} mm/s",
                                    #                            foreground=(self.col_red if bad else self.col_green))
                                    # Nếu bạn dùng 1 label "sAcc: —" duy nhất:
                                    self.lbl_sacc.config(
                                        text=f"sAcc: {s_mms} mm/s",
                                        foreground=(
                                            self.col_red if bad else self.col_green
                                        ),
                                    )
